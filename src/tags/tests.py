...
class TaggedItemTestCase(TestCase):
    ...
    def test_create_via_content_type(self):
        c_type = ContentType.objects.create(app_label="playlist", model="playlist")
        # c_type.model_class()
        tag_a = TaggedItem.objects.create(
            content_type=c_type, object_id=1, tag="new-tag"
        )
        self.assertIsNotNone(tag_a.pk)
        tag_b = TaggedItem.objects.create(
            content_type=c_type, object_id=100, tag="new-tag2"
        )
    ...
    def test_create_via_apap_loader_content_type(self):
    ...
    def test_related_field_via_content_type(self):
        ...
        tag_qs = TaggedItem.objects.filter(
            content_type=c_type, object_id=self.ply_obj.id
        )
